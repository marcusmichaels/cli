...
t.test('shouldOmit method', t => {
  t.test('dev dependency with dev omit', t => {
    ...
    node.dev = true
    t.equal(node.shouldOmit(['dev']), true, 'should omit dev dependency when dev is omitted')
    t.equal(node.shouldOmit(new Set(['dev'])), true, 'should work with Set input')
    t.equal(node.shouldOmit(['optional']), false, 'should not omit dev dependency when only optional is omitted')
    t.end()
  })
  ...
  t.test('optional dependency with optional omit', t => {
    ...
    node.optional = true
    t.equal(node.shouldOmit(['optional']), true, 'should omit optional dependency when optional is omitted')
    t.equal(node.shouldOmit(['dev']), false, 'should not omit optional dependency when only dev is omitted')
    t.end()
  })
  ...
  t.test('peer dependency with peer omit', t => {
    ...
    node.peer = true
    t.equal(node.shouldOmit(['peer']), true, 'should omit peer dependency when peer is omitted')
    t.equal(node.shouldOmit(['dev']), false, 'should not omit peer dependency when only dev is omitted')
    t.end()
  })
  ...
  t.test('devOptional dependency', t => {
    ...
    t.equal(node.shouldOmit(['dev', 'optional']), true, 'should omit devOptional when both dev and optional are omitted')
    t.equal(node.shouldOmit(['dev']), false, 'should not omit devOptional when only dev is omitted')
    t.equal(node.shouldOmit(['optional']), false, 'should not omit devOptional when only optional is omitted')
    t.end()
  })
  ...
  t.test('regular dependency', t => {
    ...
    t.equal(node.shouldOmit(['dev', 'optional', 'peer']), false, 'should never omit regular dependencies')
    t.end()
  })
  ...
  t.test('empty omit set', t => {
    ...
    node.dev = true
    t.equal(node.shouldOmit([]), false, 'should not omit anything when omit list is empty')
    t.equal(node.shouldOmit(), false, 'should handle undefined omit list')
    t.end()
  })
